...
func (pg *PostgresWorkoutStore) CreateWorkout(workout *Workout) (*Workout, error) {
	...
	err = tx.QueryRow(ctx, query, workout.UserID, workout.Title, workout.Description, workout.DurationMinutes, workout.CaloriesBurned).Scan(&workout.ID)
	if err != nil {
		return nil, err
	}
	...
	for i := range workout.Entries {
		...
		RETURNING id
		`
		err = tx.QueryRow(ctx, query, workout.ID, entry.ExerciseName, entry.Sets, entry.Reps, entry.DurationSeconds, entry.Weight, entry.Notes, entry.OrderIndex).Scan(&entry.ID)
		if err != nil {
			return nil, err
		}
	}
...
func (pg *PostgresWorkoutStore) UpdateWorkout(workout *Workout) error {
	...
	result, err := tx.Exec(context.Background(), query, workout.Title, workout.Description, workout.DurationMinutes, workout.ID)
	if err != nil {
		return err
	}
	...
	_, err = tx.Exec(context.Background(), `DELETE FROM workout_entries WHERE workout_id = $1`, workout.ID)
	if err != nil {
		return err
	}
...
func (pg *PostgresWorkoutStore) GetWorkoutOwner(workoutID int64) (int, error) {
	...
	err := pg.db.QueryRow(context.Background(), query, workoutID).Scan(&userID)
	if err != nil {
		return 0, err
	}
